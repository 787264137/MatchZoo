...
>>>>>>> 6636718fe1739e29021c42bc19be0f43de359b94
    ...
    for tag in input_conf.keys():
        if 'phase' not in input_conf[tag]:
            continue
<<<<<<< HEAD
        if input_conf[tag]['phase'] == 'TRAIN': # tag : train
=======
        if input_conf[tag]['phase'] == 'TRAIN':  # tag : train
>>>>>>> 6636718fe1739e29021c42bc19be0f43de359b94
...
<<<<<<< HEAD
                    genfun,
                    steps_per_epoch = display_interval,  # batch size
                    epochs = 1,
                    shuffle=False,
                    verbose = 0
                ) #callbacks=[eval_map])
            ...
            lossy.append(history.history['loss'][0])
        ...
        for tag, generator in eval_gen.items():
            ...
            if tag == 'test':
                mapp.append(res['map']/num_valid)

        if (i_e + 1) % save_weights_iters == 0:
            np_mapp = np.array(mapp)
...
=======
            ...
            t_loss.append(history.history['loss'][0])

            # if (i_e + 1) % save_weights_iters == 0:
            #     np_lossy = np.array(t_loss)
            #     x = range(len(np_lossy))
            #     plt.figure()
            #     plt.plot(x, np_lossy, color = 'red')
            #     plt.title('model loss')
            #     plt.ylabel('loss')
            #     plt.xlabel('iters')
            #     plt.ylim(0, 1.75)
            #     plt.savefig('z_figures/loss-%d.png' % (i_e + 1))
        ...
        for tag, generator in eval_gen.items():
            ...
            if tag == 'valid':
                print('valid')
                # y_pos = [y_pred[::i,:] for i in range(0,len(y_pred),2)]
                # y_neg = [y_pred[::i,:] for i in range(1,len(y_pred),2)]
                y_pred = tf.convert_to_tensor(y_pred_all)
                y_pos = Lambda(lambda a: a[::2, :], output_shape=(1,))(y_pred)
                y_neg = Lambda(lambda a: a[1::2, :], output_shape=(1,))(y_pred)
                loss = K.maximum(0., 1.0 + y_neg - y_pos)
                mean_loss = K.mean(loss)
                v_loss.append(mean_loss)
            if tag == 'test':
                print('test')
                # y_pos = [y_pred[::i,:] for i in range(0,len(y_pred),2)]
                # y_neg = [y_pred[::i,:] for i in range(1,len(y_pred),2)]
        ...
        #     plt.xlabel('iters')
        #     plt.ylim(0, 0.7)
        #     plt.savefig('z_figures/map-%d.png' % (i_e + 1))

        if (i_e + 1) % save_weights_iters == 0:
            np_t_lossy = np.array(t_loss)
            np_v_lossy = np.array(v_loss)
            np_c_lossy = np.array(c_loss)
            x = range(len(np_t_lossy))
            plt.figure()
            plt.plot(x, np_t_lossy, color = 'r')
            plt.plot(x, np_v_lossy,color = 'g')
            plt.plot(x, np_c_lossy,color = 'b')
            label = ["训练集", "验证集", "测试集"]
            ...
            plt.savefig('z_figures/loss3-%d.png' % (i_e + 1))
...
>>>>>>> 6636718fe1739e29021c42bc19be0f43de359b94
    ...
    weights_file = 'weights/knrm.wikiqa.weights.100'
    model = load_model(config)
...
<<<<<<< HEAD
                        dinfo = sorted(dinfo.items(), key=lambda d:d[1][0], reverse=True)  # 根据score对于结果进行排序
