...
def buildAllWordsVecs(data, stopkey, model):
    ...
    for index in range(len(idList)):
        ...
        data_vecs.to_csv('./test_data/vecs/wordvecs_' + str(id) + '.csv', index=False,encoding="utf-8")
        print ("document ", id, " well done.")

def main():
    # 读取数据集
    dataFile = './test_data/data6.txt'
    data = pd.read_table(dataFile,encoding='utf-8')
    # 停用词表
    stopkey = [w.strip() for w in codecs.open('./stopwords.txt', 'r',encoding='utf-8').readlines()]
    # 词向量模型
    inp = './model_file/wiki.zh.text.vector'
